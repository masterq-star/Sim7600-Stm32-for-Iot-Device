...
char lcdContent[LCD_HEIGHT][LCD_WIDTH + 1];  // +1 for null-terminator
...
void updateLcdContent(int row, const char *newContent)
{
    ...
    if (strncmp(lcdContent[row], newContent, LCD_WIDTH) != 0)
    {
        // Update the content
        strncpy(lcdContent[row], newContent, LCD_WIDTH);
        lcdContent[row][LCD_WIDTH] = '\0';  // Ensure null-termination

        // Update the LCD
        setCursor(0, row);
        lcd_send_string(lcdContent[row]);
    }
}
